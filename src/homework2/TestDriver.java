...
public class TestDriver {
    ...
    private void createNode(String nodeName, String cost) {
        WeightedNode weightedNode = new WeightedNode(nodeName, Integer.parseInt(cost));
        nodes.put(nodeName, weightedNode);
    ...
    private void addNode(String graphName, String nodeName) {
        ...
        WeightedNode weightedNode = nodes.get(nodeName);
        if (graph == null || weightedNode == null) {
//          throw new CommandException("addNode Command got non exist graph or node");
            return;
        }
        graph.addNode(weightedNode);
    ...
    private void addEdge(String graphName, String parentName, String childName) {
        ...
        if (graph == null || parentNode == null || childNode == null) {
//            throw new CommandException("addEdge got non exist graph or node");
            return;
        }
    ...
    private void listNodes(String graphName) {
        Graph graph = graphs.get(graphName);
        if (graph == null) {
//            throw new CommandException("listNode got a non valid graph name");
            return;
        }
        Iterator<WeightedNode> iterator = graph.getNodesIterator();
        ArrayList<String> nodesName = new ArrayList<>();
        while (iterator.hasNext()) {
            nodesName.add(iterator.next().getName());
        }
    ...
    private void listChildren(String graphName, String parentName) {
        Graph graph = graphs.get(graphName);
        WeightedNode weightedNode = nodes.get(parentName);
        if (graph == null || weightedNode == null) {
            return;
        }
        Iterator<WeightedNode> iterator = graph.getNodeChildrenIterator(weightedNode);
        ArrayList<String> childrenName = new ArrayList<>();
        while (iterator.hasNext()) {
            childrenName.add(iterator.next().getName());
        }
    ...
    private void findPath(String graphName, List<String> sourceArgs,
        ...
        List<WeightedNodePath> startPaths = new LinkedList<>();
        for (String startNodeName : sourceArgs) {
            WeightedNode sourceNode = nodes.get(startNodeName);
            WeightedNodePath startPath = new WeightedNodePath(sourceNode);
            if (!startPaths.contains(startPath)) {
                startPaths.add(startPath);
            }
        }

        List<WeightedNode> goalNodes = new LinkedList<>();
        for (String goalNodeName : destArgs) {
            WeightedNode goalNode = nodes.get(goalNodeName);
            if (!goalNodes.contains(goalNodeName)) {
                goalNodes.add(goalNode);
            }
        }
        WeightedNodePath shortestPath = PathFinder.FindShortestPath(graph, startPaths, goalNodes);
        if (shortestPath != null) {
            Iterator<WeightedNode> shortestPathIterator = shortestPath.iterator();
            output.print("shortest path in " + graphName + ":");
            while (shortestPathIterator.hasNext()) {
                output.print(" " + shortestPathIterator.next().getName());
            }
            output.println("");
        } else {
